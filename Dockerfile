...
COPY configure-mysql.bash /opt/docker-arm-mysql/

## Run MySQL configuration for docker script
RUN /opt/docker-arm-mysql/configure-mysql.bash

## Upload script to initialise MySQL
COPY init-insecure.bash /opt/docker-arm-mysql/

## Run script to initialize MySQL
RUN /opt/docker-arm-mysql/init-insecure.bash
